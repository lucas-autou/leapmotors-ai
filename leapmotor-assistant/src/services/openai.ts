...
const SYSTEM_PROMPT = `Você é a LEA, recepcionista digital da Leapmotor Brasil. Seja direta, concisa e útil.

ESTILO DE RESPOSTA:
- Respostas de 1-2 frases curtas quando possível
- Vá direto ao ponto principal
- Use linguagem natural e amigável
- 1 emoji por mensagem máximo
- Sempre ofereça próximo passo concreto
...
class OpenAIService {
  ...
  async getResponse(userMessage: string, conversationHistory: Array<{role: string, content: string}> = []): Promise<{
    ...
    try {
      ...
      const completion = await this.client.chat.completions.create({
        ...
        max_tokens: 150, // Reduzido para respostas mais concisas
        presence_penalty: 0.1, // Reduzido para permitir repetição de conceitos úteis
        frequency_penalty: 0.1,
        top_p: 0.85 // Reduzido para respostas mais diretas
      });
  ...
  private getDemoResponse(userMessage: string): string {
    ...
    if (lowerMessage.includes('olá') || lowerMessage.includes('oi') || lowerMessage.includes('bom dia') || lowerMessage.includes('boa tarde')) {
      return 'Oi! Sou a LEA da Leapmotor 😊 Como posso ajudar? Quer conhecer nossos carros elétricos?';
    }

    // Vehicle queries
    if (lowerMessage.includes('veículo') || lowerMessage.includes('carro') || lowerMessage.includes('modelo')) {
      return 'Temos B10, T03 e C10 elétricos! 🚗 Qual te interessa mais?';
    }

    if (lowerMessage.includes('b10') || lowerMessage.includes('b 10') ||
        lowerMessage.includes('be10') || lowerMessage.includes('ben10') ||
        lowerMessage.includes('ben 10') || lowerMessage.includes('b-10')) {
      return 'B10: SUV compacto, 420km autonomia, R$ 239.990! 🚗 Quer test-drive?';
    }

    if (lowerMessage.includes('t03') || lowerMessage.includes('t 03')) {
      return 'T03: ideal pra cidade, 280km autonomia, R$ 169.990! 🌱 Quer experimentar?';
    }

    if (lowerMessage.includes('c10') || lowerMessage.includes('c 10')) {
      return 'C10: SUV premium, 420km autonomia, R$ 299.990! ✨ Posso agendar test-drive?';
    }
